...
each ruleset actually infers ---, with a case study assessing the ECMO~0.3.0
ontology against those regimes, and with a survey of openly available
reasoners.
\end{abstract}
...
exercises one construct per axiom block, loaded into five repositories
(\texttt{rdfsplus-optimized}, \texttt{owl-horst-optimized},
\texttt{owl-max-optimized}, \texttt{owl2-ql-optimized},
\texttt{owl2-rl-optimized}; \texttt{owl:sameAs} enabled) over two runs, and
queried with inference switched on. The Horst repository was replaced by the
RL one between the two runs; all other results were identical across runs.
The key trick is an
...
\toprule
& \textbf{Construct tested} & \textbf{RDFS-Plus} & \textbf{Horst}$^{e}$ &
...
Q6 & hasValue, someValuesFrom, allValuesFrom & \tno & \tyes & \tyes & \tno & \tyes$^{f}$\\
...
\midrule
Q10 & implicit triples                      & 193 & 337 & 361 & 1596 & 1263\\
\bottomrule
\end{tabular}
...
$^{d}$~beyond the OWL~2~QL profile.\quad
$^{e}$~first run only.\\
$^{f}$~includes a classification that the OWL~2~RL profile does not license,
see below.
\end{table}
...
qualified existential on the subclass side.

OWL~2~RL is the only regime that passes every test. On Q1--Q7 it returns
exactly what OWL-Max returns, plus \texttt{owl:Thing} typing; it is the only
ruleset to fire property chains (Q8: the affiliation of \texttt{ex:alice}
propagates from the research group to department, faculty and university) and
keys (Q9: the two individuals sharing an ORCID are merged, and the h-index
stated on one becomes a property of the other). On this test bed, then,
\texttt{owl2-rl} strictly contains \texttt{owl-max}, which strictly contains
\texttt{owl-horst} --- although the three are not nested by design.

Five observations deviate from expectations.
\begin{enumerate}
  \item \textbf{\texttt{rdfsplus-optimized} applies neither domain nor
    ...
    for it, and \texttt{owl2-rl-optimized} applies domain and range
    correctly, so the problem is not common to all optimised rulesets. To be
    confirmed by querying range and subproperty inferences separately, and by
    repeating the run on the non-optimised ruleset.
  ...
  \item \textbf{OWL~2~RL classifies beyond its profile.} The test class
    \texttt{Supervisor} is declared \emph{equivalent} to
    $\exists\rn{supervises}.\cn{PhDStudent}$; since OWL~2~RL admits an
    existential only in subclass position, the axiom lies outside the
    profile. The ruleset nevertheless applies the classifying rule and types
    \texttt{ex:alice} as a supervisor. This is the behaviour of the RL/RDF
    rules, which match triples regardless of the syntactic position they came
    from: the profile restricts what is \emph{guaranteed}, not what fires.
  \item \textbf{The two OWL~2 rulesets materialise far more than the others}
    (1596 implicit triples for QL, 1263 for RL, against 193--361). Both type
    every individual as \texttt{owl:Thing}, which the other optimised
    rulesets do not; QL still produces more than RL despite being the weaker
    regime.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Materialisation cost varies sharply between rulesets}, and is
    not monotone in apparent expressivity: OWL~2~QL materialises more than
    OWL~2~RL, and both several times more than OWL-Max.
  \item \textbf{For ontologies that need more than RDFS-Plus, OWL~2~RL is the
    safe default.} It was the only regime to pass every test, and the only one
    covering property chains and keys.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Case study: ECMO 0.3.0 against the GraphDB rulesets}
\label{sec:ecmo}

This section applies the analysis of Section~\ref{sec:graphdb} to a real
ontology network: ECMO~0.3.0, the JRC ontology for emergency and crisis
management, built on the Sendai Framework terminology and aligned to
DOLCE+DnS Ultralite (DUL) through d0. The assessment is based on the backbone
modules (hub, core, core pattern, foundation, disaster, risk); the remaining
domain modules and the file of inferred axioms were not inspected, so the
picture is indicative rather than a complete profile.

\subsection*{Constructs used and ruleset support}
...
\begin{tabular}{@{}p{3.2cm}p{4.9cm}cccccc@{}}
\toprule
\textbf{Construct} & \textbf{Example in ECMO} & \textbf{RDFS} & \textbf{RDFS+} &
  \textbf{Horst} & \textbf{Max} & \textbf{QL} & \textbf{RL}\\
\midrule
Subclass, subproperty, domain, range & throughout
  & \tyes & \tyes & \tyes & \tyes & \tyes & \tyes\\
Inverse properties & dozens of named pairs, e.g.\ \texttt{hasHazardDeterminant}
  & & \tyes & \tyes & \tyes & \tyes & \tyes\\
Transitive properties & \texttt{subsumedUnder}, \texttt{specializes},
  \texttt{includesEvent}
  & & \tyes & \tyes & \tyes & \tno & \tyes\\
Symmetric property & \texttt{isComplementaryTo}
  & & \tyes & \tyes & \tyes & \tyes & \tyes\\
\texttt{hasValue}, superclass side
  & $\cn{DirectlyAffectedPopulation} \sub \exists\rn{hasAffectednessType}.\{\ind{Direct}\}$
  & & & \tyes & \tyes & \tno & \tyes\\
\texttt{owl:sameAs} & the HIPS hazard individuals
  & & & \tyes & \tyes & \tno & \tyes\\
Defined class (intersection, nested $\exists$, \texttt{hasValue})
  & $\cn{Disaster}$, see below
...
Disjointness & \texttt{Frequent}/\texttt{InfrequentDisaster};
  DUL: $\cn{Description} \sqcap \cn{Situation}$ $\sqsubseteq \bot$
...
\smallskip
\raggedright
$^{a}$~in the classifying direction ($\sqsupseteq$) only.
\end{table}

\noindent
The defined class that carries most of the semantic weight is
\[
...
\]
Only its right-to-left half is rule-expressible, and only OWL~2~RL guarantees
it; it is what lets a materialising store classify a hazardous event as a
disaster from its reported impacts.

\subsection*{DL expressivity}

The constructs above --- nominals through \texttt{hasValue}, inverse and
transitive roles, role chains, qualified cardinality, union in superclass
position, datatypes --- place ECMO in $\SROIQ^{(\mathcal{D})}$, i.e.\ OWL~2~DL,
and outside all three OWL~2 profiles.

Strictly, one axiom takes it out of OWL~2~DL. \texttt{foundation:assesses} is
defined by a property chain and is therefore a \emph{non-simple} role, and
...
\end{lstlisting}
Since ${\geq}1\,R.C \equiv \exists R.C$ and existentials are permitted on
non-simple roles, replacing the restriction with
\texttt{owl:someValuesFrom dul:Entity} preserves the meaning and restores
OWL~2~DL conformance.

\subsection*{What each ruleset delivers for ECMO}

\begin{itemize}
  \item \textbf{RDFS-Plus is the minimum useful regime.} ECMO's own design
    rests on the Description-and-Situation subsumption hierarchy expressed
    through the transitive \texttt{subsumedUnder}/\texttt{specializes}, on
    systematically named inverses, and on domain/range typing. In view of the
    anomaly reported in Section~\ref{sec:graphdb}, domain and range inference
    must be verified on whichever ruleset is deployed.
  \item \textbf{OWL-Horst} adds the \texttt{hasValue} classifications of the
    affected-population and risk-profile subclasses, and merges the HIPS
    hazard individuals through \texttt{owl:sameAs}.
  \item \textbf{OWL-Max} adds the union, making every event and situation a
    \texttt{d0:Eventuality} --- largely redundant with the domain axioms on
    that class.
  \item \textbf{OWL~2~RL} is the only ruleset that fires the property chains
    linking \texttt{assesses} to DUL's setting and aboutness relations, that
    reliably classifies events as \texttt{Disaster}, and that checks the
    disjointness axioms ECMO relies on as coherence guards. The test bed
    confirms that \texttt{owl2-rl-optimized} fires chains and applies domain
    and range, and that it applies classifying rules even when the axiom as a
    whole --- an equivalence with an existential --- lies outside the RL
    profile, which is the shape of ECMO's \texttt{Disaster} definition.
  \item \textbf{OWL~2~QL} is unsuitable: without transitivity the whole
    \texttt{subsumedUnder} hierarchy is lost, together with chains and
    \texttt{hasValue}.
  \item \textbf{No ruleset} exploits the union in superclass position, the
    superclass-side existentials (the four determinants of \texttt{Risk},
    \texttt{Signal}, \texttt{EventValidation}), or the cardinality axiom.
    Their consequences require a DL reasoner.
\end{itemize}

\subsection*{Deployment notes}

\begin{itemize}
  \item \textbf{Imports are not followed.} GraphDB does not resolve
    \texttt{owl:imports}; all modules, and DUL/d0, must be loaded explicitly.
    Without DUL the property chains have nothing to fire on and the DUL
    disjointness guards are absent.
  \item \textbf{The import closure is incomplete as distributed.} The catalog
    maps the eight alignment modules to a \texttt{0.3.0-alignments/}
    directory that is not present, and the \texttt{frames} module imported by
    \texttt{ecmo-core} has no catalog entry (presumably
    \texttt{ecmo-frame-model.ttl}).
  \item \textbf{Recommended set-up:} OWL~2~RL in the store, complemented by an
    offline DL reasoner (Konclude or HermiT) for the non-Horn consequences.
    The distributed \texttt{ecmo-inferredaxioms.ttl} appears to be the output
    of such a run; if so, loading it alongside the modules closes most of the
    remaining gap (not verified).
  \item \textbf{A modelling point to review.} \texttt{risk:Risk} is asserted
    as a subclass of \texttt{hazard:Hazard}, so every rule-based regime types
    every risk as a hazard --- at odds with ECMO's own reading of risk as a
    function of hazard, exposure, vulnerability and capacity.
\end{itemize}

\noindent
A complete, rather than sampled, profile can be obtained with ROBOT once the
catalog is fixed; the RL report lists exactly the axioms a GraphDB OWL~2~RL
repository cannot fully exploit, the DL report should flag the cardinality
violation above.
